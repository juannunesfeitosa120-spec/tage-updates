...
export async function loadReleasePolicy(
  ...
  const [deviceResult, policyResult] = await Promise.all([
    ...
    supabase
      ...
      .select(
        'latest_version,minimum_supported_version,blocked_versions',
      )
      .eq('id', true)
      .maybeSingle(),
  ]);

  if (deviceResult.error || policyResult.error) return fallback;
  const device = Array.isArray(deviceResult.data)
    ? deviceResult.data[0]
    : deviceResult.data;
  ...
  return {
    channel:
      device?.enabled && device?.channel === 'beta' ? 'beta' : 'stable',
